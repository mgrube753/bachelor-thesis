...
INPUT_PATH = os.path.join(
    BASE_PROJECT_PATH, "20_experiments/60_analyses/csv/quantitative/exp1"
)
...
for path in [
    output_dir_exp1a,
    output_dir_exp1b,
    output_plot_dir_exp1a,
    output_plot_dir_exp1b,
]:
...
def get_scores_from_df(df, input_source, layer, llm, prompt_type):
    ...
    if not matching_rows.empty:
        return (
            matching_rows["cosine_similarity"].iloc[0],
            matching_rows["adherence_score"].iloc[0],
        )

    fallback_rows = df[(df["input_source"] == input_source) & (df["layer"] == layer)]
    if not fallback_rows.empty:
        return (
            fallback_rows["cosine_similarity"].mean(),
            fallback_rows["adherence_score"].mean(),
        )
...
def analyze_free_text_questions():
    ...
    question_files = [
        f
        for f in glob.glob(os.path.join(base_path, "**/*_question.txt"), recursive=True)
    ...
    for file_path in question_files:
        ...
        try:
            ...
            if "Antwort:" in content and "Antwortmöglichkeiten:" not in content:
                input_source, layer, llm, prompt_type = extract_info_from_path(
                    file_path
                )
                ...
                cosine_sim, adherence_score = get_scores_from_df(
                    df, input_source, layer, llm, prompt_type
                )
...
print(
    f"  Exp1a: {total_counts['exp1a']} | Exp1b: {total_counts['exp1b']} | Total: {sum(total_counts.values())}"
)
...
for i, item in enumerate(
    sorted(free_text_data, key=lambda x: (x["experiment"], x["path"])), 1
):
    print(
    ...
    if item["cosine_similarity"] is not None and item["adherence_score"] is not None:
        print(
            f"   Scores: Cosine={item['cosine_similarity']:.3f}, Adherence={item['adherence_score']:.3f}"
        )
    else:
...
if not valid_data.empty:
    summary_stats = (
        valid_data.groupby("experiment")[["cosine_similarity", "adherence_score"]]
        .agg(["mean", "std", "count"])
        .round(4)
    )
    simple_summary = (
...
def create_dual_heatmap(cosine_pivot, adherence_pivot, title_base, exp_label):
    ...
    ax1.set_title(
        f"Cosine Similarity (-1 to 1): {title_base} ({exp_label})\nValues: Mean (Std)",
        fontsize=12,
        fontweight="bold",
        pad=15,
    )
    ...
    ax2.set_title(
        f"Adherence Score (0 to 1): {title_base} ({exp_label})\nValues: Mean (Std)",
        fontsize=12,
        fontweight="bold",
        pad=15,
    )
...
adherence_pivot = pd.pivot_table(
    exp1a_df,
...
fig = create_dual_heatmap(
    llm_vs_input_source_cos_sim_1a, adherence_pivot, "LLM vs Input Source", "Exp 1a"
)
...
adherence_pivot_prompt = pd.pivot_table(
    exp1a_df,
    values="adherence_score",
    index="llm",
    columns="prompt_type",
...
fig = create_dual_heatmap(
    llm_vs_prompt_type_cos_sim_1a,
    adherence_pivot_prompt,
    "LLM vs Prompt Type",
    "Exp 1a",
)
...
fig = create_dual_heatmap(
    llm_vs_input_source_cos_sim_1a,
    pd.pivot_table(
        exp1a_df,
        values="adherence_score",
        index="llm",
...
adherence_pivot_1b = pd.pivot_table(
    exp1b_df,
    values="adherence_score",
    index="llm",
    columns="prompt_type",
...
fig = create_dual_heatmap(
    llm_vs_prompt_type_cos_sim_1b, adherence_pivot_1b, "LLM vs Prompt Type", "Exp 1b"
)
...
if exp1a_no_source_df is not None:
    ...
    comparison_means = comparison_df.groupby("input_source")[
        ["cosine_similarity", "adherence_score"]
    ].mean()
    ...
    comparison_summary = comparison_df.groupby("input_source")[
        ["cosine_similarity", "adherence_score"]
    ].agg(["mean", "std", "median", "min", "max", "count"])
    ...
    print(
        "Skipping Input Source vs No Source comparison - exp1a_no_source data not available."
    )
