    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item Aktives Lernen $\to$ gezielte Fragen
                \note[item]{Um \textbf{aktives Lernen} zu fördern und nicht nur Texte zu lesen, sind gezielte \textbf{Kontrollfragen} sinnvoll.}
            \item Anspruchsvolle Fragen $\to$ zeitaufwendig \cite{steuer_i_2021} 
                \note[item]{Eine Menge an \textbf{anspruchsvollen Fragen} zu erstellen ist jedoch \textbf{zeitaufwendig}.}
        \end{itemize}
        \vspace{.5em}
...
\begin{frame}{Motivation \& Problemstellung II}
    \begin{itemize}
        \item \textbf{Problemstellung:} Qualität oft unklar $\to$ mangelnde systematische Evaluation über linguistische Tests hinaus \cite{horbach_linguistic_2020} 
            ...
            \note[item]{P1: man muss auch \textbf{pädagogische Eignung} bewerten, aber}
        \begin{itemize}
            \item Selten pädagogische Bewertung \cite{zhuge_twinstar_2025,blobstein_angel_2023,scaria_how_2024}
                \note[item]{P1.1: manchmal nur \textbf{linguistische Metriken} genutzt, oder eben Expertenbewertung \textbf{ohne Bloom} \\}
            \item Mangelnde Performance bei höheren kognitiven Levels \cite{elkins_how_2023,scaria_how_2024}
                \note[item]{P1.2: zu \textbf{alte} Modelle... \\}
            \item Relevanz $\neq$ Inhaltstreue \cite{elkins_how_2023,hang_mcqgen_2024,mi_comparative_2024}
                \note[item]{P1.3: \textbf{Korrektheit} von Fragen \textbf{elementar} in Bildung; \\ Inhaltstreue oft nur als \textbf{Future Work}}
        \end{itemize}
        \vspace{.5em}
...
\begin{frame}{Bloom's revised Taxonomy}
    ...
    \end{itemize}
            \note[item]{\textbf{Remembering:} Abruf von Faktenwissen -- \\\textit{wiedergeben, nennen}}
            \note[item]{\textbf{Understanding:} Bedeutung von Anweisungen erkennen -- \\\textit{erklären, beschreiben, zusammenfassen}}
            \note[item]{\textbf{Applying:} Wissen in gegebenen Situationen anwenden -- \\\textit{anwenden, benutzen, durchführen}}
            \note[item]{\textbf{Analyzing:} Material in Komponenten zerlegen -- \\\textit{untersuchen, vergleichen, kategorisieren}}
            \note[item]{\textbf{Evaluating:} Urteile basierend auf Kriterien fällen -- \\\textit{bewerten, beurteilen, kritisieren}}
            \note[item]{\textbf{Creating:} Elemente zu Ganzem zusammenfügen -- \\\textit{entwerfen, planen, entwickeln}}
        \begin{figure}[htpb]
            \centering
...
\begin{frame}{Herausforderungen der Literatur}
    \begin{itemize}
        \item \textbf{Veraltete Modelle} -- Kein Reasoning
            \note[item]{P1: basiert auf User Prompt $\Rightarrow$ Anforderungen \textbf{brainstormen}, \\ zuvor \textbf{nicht genutzt}, sondern \textbf{outdated} Models (GPT-4)}
        \item \textbf{Halluzinationen} -- Falsche Ausgaben
            \note[item]{P2: Kann man minimieren durch \textbf{Kontext}}
        \item \textbf{Inhaltstreue} -- Forschung nötig
            \note[item]{P3: Inhaltstreue eher \textbf{Nebensache}, mehr \textbf{Bloom} Fokus, \textbf{linguistische} Metriken}
        \item \textbf{Quantitative Metriken} -- Linguistischer Fokus
            \note[item]{P4: Problematisch, da grundlegend \textbf{linguistisch} \\ $\Rightarrow$ \textbf{Expertenbewertung} notwendig bei AQG}
        \item \textbf{Expertenbewertung} -- Selten, da aufwendig
            \note[item]{P5: \textbf{Selten} ausgiebig performt, weil aufwendig}
        \item \textbf{Bloom's Taxonomy} -- Level Alignment \& \\ hohe kognitive Levels
            \note[item]{P6: Hohe Levels \textbf{misslingen} in RW; \\ auch \textbf{nicht immer Bloom} benutzt (dann eher schlecht)}
    \end{itemize}
\end{frame}
...
\begin{frame}{How to: Fleiss' Kappa}
    \begin{itemize}
        ...
        \end{align*}
        \textcolor{gray}{$p_o$ beobachtete Übereinstimmung, \\ $p_e$ erwartete Übereinstimmung durch Zufall, \\
        $N$ Anzahl der Fragen, \\
        $n$ Anzahl der Rater}
            \note[item]{$n_{ij}$ \textbf{\#Rater}, die für Frage $i$ das Rating $j$ abgegeben haben}
            \note[item]{$k$ Menge der \textbf{Auswahlmöglichkeiten} 0,2,4,6,8,10}
            % \note[item]{\textbf{Nenner} bei $p_o$ ist maximale Anzahl \textbf{möglicher Übereinstimmungen} von Raterpaaren über alle Fragen}
            \note[item]{$p_e$ wird jede \textbf{Zählung quadriert}, die werden \textbf{summiert} (wie viele Rater haben in Rating $j$ Zustimmung gegeben)}
        \begin{align*}
...
\begin{frame}{Beispiel: Fleiss' Kappa}
    \begin{minipage}{.45\linewidth}
        ...
        \begin{table}
            \begin{tabular}{c|ccc}
                ...
                \midrule
                1 & \textcolor{green!70}{Y} & \textcolor{green!70}{Y} & \textcolor{green!70}{Y} \\
                2 & \textcolor{red}{N} & \textcolor{green!70}{Y} & \textcolor{red}{N} \\
                \bottomrule
            \end{tabular}
        \end{table}
        \vspace{-1em} 
    ...
    \begin{minipage}{.53\linewidth}
        \centering
        $N=2$ Fragen, $n=3$ Raters: \vspace{.75em} \\
        $\displaystyle p_o=\frac{3^2+0^2+1^2+2^2-2\cdot 3}{2\cdot 3\cdot (3-1)}=\uline{0.66}$ \vspace{2em} \\
        Einsetzen in $\kappa$-Gleichung: \vspace{.75em} \\
        $\displaystyle\kappa=\frac{0.66 - 0.55}{1 - 0.55}=\uline{0.25}$
    \end{minipage}
\end{frame}
...
\begin{frame}{Experimentelles Design I}
    \begin{itemize}
        ...
        \begin{itemize}
            \item Script -- \enquote{Referenzarchitekturen} (DE)
            \item Transcript -- Audio-zu-Text (DE)  
            \item Textbook -- \enquote{Computer Networks} (EN)
        \end{itemize}
        ...
        \item 2 Prompt-Strategien:
            \begin{itemize}
                \item \textbf{Einfach:} Weniger Anweisungen, keine Strukturierung, aber Formatvorlage + Material
                    \note[item]{P2.1: \textbf{Generiere} zum ISO-OSI Modell; korrekte Antwort \textbf{klar gekennzeichnet} sein; \textbf{keine Strukturierung}; Frage \textbf{klar \& präzise}, soll \textbf{kein weiterer Text} generiert werden}
                \item \textbf{Komplex:} Rollenzuweisung, detaillierte Anweisungen, fordert nicht-triviales und kritisches Denken, Struktur durch Docstrings, Formatvorlage + Material 
                    \note[item]{P2.2: \textbf{Experte} in Generieren von kognitiv anspruchsvollen Fragen;
                    \textbf{Untersuchen} des Texts \& \textbf{Erfassen} des Wichtigen; nur \textbf{Fließtext}}
            \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]{Evaluations-Methodik}
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \item[] \textit{\textcolor{gray}{Relevance, Clarity, Answerability, Challenging, Value, Language \& Correctness\,/\,Manipulation Handling}}
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}[allowframebreaks]{Experimentelles Design}
    ...
    \begin{align*}
        ...
        \textcolor{gray}{4 \text{ LLMs}\times 6 \text{ Levels}} &= \textcolor{gray}{24 \text{ Fragen}} \\
        \textcolor{gray}{4 \text{ LLMs}\times 2 \text{ Fragetypen}\times 6 \text{ Levels}} &= \textcolor{gray}{48 \text{ Fragen}}
    \end{align*}
\end{frame}
...
\begin{frame}{Hauptergebnisse zu RQ1 I}
    \begin{itemize}
        \item \textbf{LLMs erstellen inhaltsgetreue Fragen} unter bestimmten Bedingungen
            \note[item]{P1: Adherence Score $\Rightarrow$ die Fragen \textbf{inhaltlich gut}\\ Sentence Transformer \textbf{ungeeignet} für langes\,/\,strukturiertes}
        \item \textbf{Common Prompt > Complex} meistens
            \note[item]{P2: Sowohl bei \textbf{quantitativ} als auch \textbf{qualitativ}; Complex nur besser in \textbf{Challenging} mit Einbußen \\
            Überladen\,/\,wenig präziser Prompt $\Rightarrow \neg$ besseres}
        \item Script vs. Transcript oft eng beieinander, wobei \\
        \textbf{Staff:} Script vs. \textbf{Supervisor:} Transcript
            \note[item]{P3: \textbf{Transcript Cap} bestes in Clarity, Answerability und Correctness, beide Gruppen \textbf{Challenging} geringstes \\
            \textbf{Script Staff} ausgeglichen mit höchstem \textbf{Total Score ebenso} bei Script, oft das effektivere Material}
        \item OpenAI \& DeepSeek \textbf{konsistenter}
            \note[item]{P4: Anthropic \& Google \textbf{variabler}... teils sprachlich \textbf{komplex} oder mit \textbf{fehlerhaften\,/\,erfundenen} Optionen}
    \end{itemize}
\end{frame}
...
\begin{frame}{Hauptergebnisse zu RQ1 II}
    \begin{itemize}
        ...
        \item \textbf{Teilweise Erkennung} von Manipulationen: \newline OpenAI \& DeepSeek beim komplexen Prompt
            % \note[item]{P2: Anthropic \& Google \textbf{folgen} hierbei häufiger}
        \item \textbf{Kleine Stichprobe} (16/56) und \textbf{geringe IRR} \\
            \note[item]{P3: Deswegen Google \& Anthropic hier \textbf{besser} \\
            Rubrik \& Pflichten der Bewerter müssen \textbf{refined} werden}
        $\Rightarrow$ Inkonsistente Trends quantitativ \& qualitativ
        \item \textbf{Bloom's Taxonomy} essenziell für gute + Anforderungs-gesteuerte Fragen
    \end{itemize}
        \note[item]{$\Rightarrow$ klappt dann gut, wenn man \textbf{klare Anweisungen} gibt + strukturiertes, präzises \textbf{Material}}
        \note[item]{Fragen \textbf{wenig Challenging} (auch nicht so sehr bei Complex Prompt) $\Rightarrow$ Wichtigkeit von \textbf{Bloom's Taxonomy} da}
\end{frame}
...
\begin{frame}{Hauptergebnisse zu RQ2 II}
    \begin{itemize}
        ...
        \item \textit{Type+Bloom}: \textbf{Google} am besten
            % \note[item]{P3: OpenAI wegen Level-6-MCQ \textbf{Widerspruch} geringer}
        \item Hohe IRR bei \textit{Bloom's Level}
            \note[item]{P4: Answerability 0.179, Challenging 0.270; aber andere gegen 0 oder Value -0.256}
            \note[item]{Braucht besseres \textbf{Rubrik-Design} (vor allem wegen Value) + \textbf{klarere Anker}}
...
\begin{frame}{Limitations\,/\,Future Work I}
    \begin{itemize}
        \item Cosine Similarity \textbf{ungeeignet} 
            \note[item]{P1: \textbf{Kein Unterschied} in Inhaltstreue zwischen Fragen mit Source Material und ohne...}
        \item Besseres \textbf{Multi-LLM-Judging} für Inhaltstreue 
            \note[item]{P2: Statt zwei Modelle und dann \textbf{Mean}, da geht mehr}
        \item \textbf{Größere Stichprobe} bei Manipulation
            \note[item]{P3: \textbf{Konsistente Auswertung}, da 16/56 zu wenig war}
        \item Klare \textbf{Ankerbeschreibungen} \& \textbf{Lernziele} integrieren 
            \note[item]{P4: 0-10 Skala doch \textbf{zu ungenau} trotz Anker}
...
\begin{frame}{Limitations\,/\,Future Work II}
    \begin{itemize}
        \item Wenn \textbf{MCQs} $\Rightarrow$ mehrstufige Pipeline 
            \note[item]{P1: Frage, richtige, falsche Optionen \textbf{separat} generieren}
        \item \textbf{Reasoning vs. Non-Reasoning} Modelle vergleichen
            \note[item]{P2: Bzw. mit und ohne den \textbf{Reasoning} Prozess}
            \note[item]{Dazu: Reasoning \textbf{schwer auswertbar}, da zunehmend proprietär; \textbf{Summaries} wenn überhaupt verfügbar}
        \item \textbf{Halluzinationen} reduzieren 
            \note[item]{Sowohl bei \textbf{Modellen} als auch durch \textbf{Prompting}}
