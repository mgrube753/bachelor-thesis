...
Firstly, the \textit{Adherence Score} proved to be a robust metric for content fidelity evaluation, while cosine similarity (sentence transformer based) failed to discriminate source-based vs.\ no-source-based generations, yielding similar means/medians for both types. Sentence transformers are unsuitable for longer educational texts, as these models are designed for sentence-level comparisons rather than the educational content structures used in this thesis.

Significantly, prompt complexity played a crucial role in question quality outcomes. Across Experiment 1a, the common prompt consistently outperformed the complex prompt in most quality dimensions -- \textit{Adherence}, \textit{Correctness}, \textit{Clarity}, \textit{Answerability}, \textit{Language}, and \textit{Value}. The complex prompt only increased \textit{Challenging} ratings but at a noticeable quality cost. This implies that vastly formulated prompts do not necessarily lead to better results.

Inter-rater agreement presented significant challenges across both experiments. Fleiss' $\kappa$ around zero for most criteria (staff-only and combined) due to wide 0-10 scale usage, where supervisors used discrete anchors while staff members used the full rating range. This reliability constraint limits the strength of qualitative ratings, though similar trends were visible in the evaluation. Student evaluation reliability (Exp2) showed slight to fair agreement for \textit{Answerability} and \textit{Challenging}, almost moderate for \textit{Bloom Rating}, but very low/negative for \textit{Relevance}, \textit{Clarity}, \textit{Value}, and \textit{Language} due to domain unfamiliarity and insufficient rubric abstraction clarity.

Implementation constraints emerged as methodological limitations. Manual DeepSeek prompting proved time-consuming and error-prone, while reasoning models' inability to control temperature settings introduced uncontrolled variability. The \textit{Value} criterion in Exp2 was problematic since it lacked explicit learning objectives integration, which was not considered during planning.

Across the experiments, linguistic complexity issues were consistent, where overly complex questions (mostly Google, Anthropic) and generic phrases e.g.\ \enquote{based on the text} (mainly Google and DeepSeek) reduced \textit{Language} and \textit{Value} ratings. These patterns suggest model-specific tendencies that affect question usability regardless of experimental conditions.
...
The quantitative analysis yielded promising results, with high \textit{Adherence Score} entries across all models and prompts (>90\% adherence). The common prompt consistently outperformed the complex prompt in content adherence across both quantitative measures and expert evaluations. Taking the qualitative analysis into account, the performance gap between the common and complex prompt was visible in most criteria, where structured materials (script and transcript) were rated higher than the Tanenbaum book excerpts.

Source material impact (Exp1a) showed that the transcript produced highest means in multiple quality dimensions (\textit{Clarity}, \textit{Answerability}, \textit{Correctness}) but lowest in \textit{Challenging}; the concise script yielded highest \textit{Total Score} due to better \textit{Challenging} balance; Tanenbaum excerpts were weakest overall (language switch + book-style). Model differentiation revealed that OpenAI and DeepSeek were more consistent in \textit{Correctness}, while Anthropic and Google were more variable, often generating linguistically complex questions (especially in the complex prompt) and occasionally incorrect or invented answer possibilities.

Manipulation handling (Exp1b) showed that OpenAI and especially DeepSeek had lower \textit{Adherence Scores} when the complex prompt was used, indicating partial detection/neutralization of injected manipulations. Anthropic and Google more often reproduced manipulations without remark. The complex prompt showed better detection of manipulations, especially for DeepSeek and OpenAI. However, the small sampled set (16/56) limits inference, and inconsistent results between supervisor and staff evaluations constrain the statistical power of findings.

All in all, the findings suggest that \acp{llm} can generate suitable questions with content fidelity, particularly when provided with clear prompts and structured source material. DeepSeek and OpenAI were the best models for this task, while Anthropic and Google were more variable in performance. The generated questions were not that challenging -- but more challenging with the complex prompt -- reflecting the need for Bloom's Taxonomy levels to be specified in prompts.
...
Bloom level specification proved critical for cognitive alignment. Specifying Bloom levels -- either Bloom Only or Type+Bloom -- resulted in high adherence to intended cognitive levels, with median scores of 10 in both cases. Without Bloom specification (Type Only), questions tended to fall into lower cognitive levels, with a median of 3 (at \textit{Understanding}), which is insufficient for demanding educational content.

Question format analysis revealed systematic differences: Open-Ended questions consistently achieved higher cognitive levels than \acp{mcq}, and even higher Bloom alignment when Bloom specification was given. \acp{mcq} were more suitable for lower cognitive levels (\textit{Remembering}, \textit{Understanding}, \textit{Applying}). Model patterns showed that without Bloom constraints, Anthropic and Google reached higher levels than DeepSeek and OpenAI; with constraints, all models converged (high adherence), with OpenAI best in Bloom Only setting and Google best in Type+Bloom setting. OpenAI occasionally underperformed in combined Type+Bloom adherence mean despite strong median, due to mismatch rating because of \acp{mcq} and Bloom level 6, which contradicts.

The student evaluation showed that \textit{Bloom's Level} had the highest inter-rater reliability, supporting the usability of clear level definitions used. However, linguistic and content-related criteria presented very low inter-rater reliability due to domain unfamiliarity and insufficient rubric clarity. Overall, Experiment 2 had more homogeneous quality outcomes than the content fidelity focus in Experiment 1.

A balanced approach combining Bloom's Taxonomy levels with appropriate question formats is essential for maximizing pedagogical effectiveness. Open-Ended questions provide the most versatile format for higher-order thinking skills, while a mix of both question types can better handle all cognitive levels.
