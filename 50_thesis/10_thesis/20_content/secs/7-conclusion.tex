...
Firstly, the quantitative results demonstrated that, based on the \textit{Adherence Score} -- as an alternate measure for content fidelity to cosine similarity --, the \acp{llm} can generate questions with high content fidelity across diverse instructional texts. However, the semantic similarity measure (sentence transformer based) failed to distinguish between source-based and no-source-based questions, yielding similar results for both types. Sentence transformers are not suitable for the texts used in this thesis, as these models are not designed for longer educational texts, but rather for sentence-level comparisons. 

Significantly, the prompt complexity played a crucial role in both quantitative and qualitative analysis at question quality outcomes, where the common prompt consistently outperformed the complex prompt in most quality dimensions, surprisingly. This was evident in the \textit{Adherence Score}, the qualitative criteria such as \textit{Clarity}, \textit{Answerability}, \textit{Language}, and \textit{Value}, and \textit{Correctness}. The complex prompt only increased the \textit{Challenging} ratings, but at a noticeable quality cost, which was not desired. This implies that a vastly formulated prompt does not necessarily lead to better results.
...
Model differentiation was also observed, with OpenAI and DeepSeek being more consistent in \textit{Adherence Score} and \textit{Correctness}, generating questions that well reflect the source material without unnecessary linguistic complexity. Anthropic and Google were more variable, with a tendency to produce linguistically complex questions, particularly in the complex prompt, and occasionally incorrect or invented answer possibilities. The \textit{Challenging} criterion was also rated lower for all models, with DeepSeek and OpenAI being worse than Anthropic and Google. 

Regarding the \textit{Manipulation Handling} in Experiment 1b, the quantitative analysis showed that OpenAI and especially DeepSeek had lower \textit{Adherence Scores} when the complex prompt was used, indicating that these models could partially detect the manipulations in the source material. In contrast, Anthropic and Google more often reproduced the manipulations without any remark. Qualitatively, the two models were rated better, in contrast to OpenAI and DeepSeek, but the small sample size of 16/56 questions in Experiment 1b and the low inter-rater reliability (Fleiss' $\kappa$ around zero for both sub-experiments) shows inconsistencies in the results. Overall, similar trends were visible in the evaluation, but since this rating task is not done every day, and the staff members used the full scale of 0-10 (instead of the desired anchors), the rubric design lacks in clarity and consistency.

The second experiment showed that specifying Bloom levels in the prompts -- either Bloom Only or coupled with a question type (Type+Bloom) -- resulted in high adherence to the intended cognitive levels, which is crucial for effective educational assessment. Without Bloom specification (Type Only), the questions tended to be at lower cognitive levels (at \textit{Understanding}\,/\,\textit{Applying}), which is not sufficient instead. Taking the question types into account, \acp{mcq} were more suitable for lower cognitive levels, while Open-Ended question can be used across all Bloom levels and achieved superior Bloom alignment, if specified. 
...
The student evaluation in Experiment 2 showed that the \textit{Bloom's Level} had the highest inter-rater reliability, but linguistic and content-related criteria such as \textit{Relevance}, \textit{Clarity}, and \textit{Language} presented very low inter-rater reliability, as well as the \textit{Value} criterion. The students were unfamiliar with the domain, which likely contributed to the low inter-rater reliability, as well as the less clear rubric design.

Across the experiments, linguistic complexity issues were observed, where overly complex questions -- mostly from Google and Anthropic -- and often occurring phrases e.g.\ \enquote{based on the text} in Experiment 1 -- mainly Google and DeepSeek -- were reducing the \textit{Language} and \textit{Value} ratings. Furthermore, the rubric designs of the experiments reduced the agreements due to its broad 0-10 scale, coupled with not sufficiently clear anchor descriptions. Unfortunately, the \textit{Value} criterion was criticized, since it was not planned to integrate explicit learning objectives for the questions, which made the evaluation more abstract and less clear.
