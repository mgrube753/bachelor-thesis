...
\begin{enumerate}
  ...
  \item Analyzing how three prompting variants -- specifying only the question format, only the targeted Bloom level, or both together -- affect the generation of questions across Bloom's revised Taxonomy and their adherence to the intended cognitive level, if a Bloom level was specified.
\end{enumerate}
...
The experimental setup for analyzing \textit{Manipulation Handling} was somewhat problematic, as the sample size for the manipulated questions was relatively small. A larger sample size, where all questions are evaluated based on the proposed \textit{Manipulation Handling} criterion, would provide more consistent insights into the \acp{llm}' reaction to manipulations, as the results were inconsistent between the supervisor and staff members, as well as quantitatively different due to the use of all questions in this manner of analysis. This also implies a redesign of the rubric to be more learning objective-oriented, since the \textit{Value} criterion was criticized for its lack of explicit learning objectives, which was not considered during the planning phase. Furthermore, clearer anchor descriptions -- e.g.\ for rating 0, rating 2, \ldots -- are needed to improve inter-rater reliability and thus the qualitative analysis of the questions.
