...
\object{Evaluation Plan} The evaluation of the generated questions will be conducted in two phases, corresponding to the two main experiments. Each phase employs a multi-modal assessment combining automated metrics, \ac{llm}-based evaluation, and expert human annotation to ensure comprehensive quality assessment.
\begin{itemize}
    \item \textbf{Experiment 1 (Content Fidelity and Error Propagation):}
    \begin{itemize}
        \item \textit{Quantitative Analysis:} In this stage, the \textit{Semantic Similarity} between generated questions and source materials will be assessed. This will be done calculating pairwise the cosine similarity on the two embedding vectors, which were created -- for the source material and the question -- by a cross-lingual RoBERTa model\footnote{\url{https://huggingface.co/T-Systems-onsite/cross-en-de-roberta-sentence-transformer}, accessed 05/09/2025}. 
        This is a sentence transformer, which offers comparable, semantic meaningful sentence embeddings \cite{reimers_sentence-bert_2019}. 
        ...
        \item \textit{Qualitative Analysis:} This phase involves a blind-test with expert-based evaluations of the generated questions. Five experts, including the main supervisor and four scientific staff members from the same university department, will rate a sampled question set using the concise rubric in Table \bfref{tab:exp1_criteria}. To analyze the inter-rater agreement, \tcr{Fleiss Kappa} will be calculated, which is suitable for more than two raters \tcr{cite}. The analysis adopts the rubric structure from \cite{mi_comparative_2024}, maintaining a maximum score of 50 points per question. The authors' rubric was created based on the criteria used by \cite{moore_assessing_2022}. However, the added \textit{Bloom's Level} criterion is replaced with \textit{Correctness} to address the objectives of this experiment. Since Experiment 1b examines the adherence to \textit{manipulated} input texts, the \textit{Correctness} category is split in half, as to be seen below:
...
Since both the human and the \ac{llm}-based evaluation shall be conducted as unbiased blind tests, the evaluators also will not be aware of the used methods to generate each question. This involves the model name, the kind of prompt used, the specified question type and the desired Bloom level for each question. Furthermore, to automatically score the \textit{Bloom's Level} -- based on the guideline in Table \bfref{tab:exp2_criteria}, the \ac{llms} as evaluators shall return solely the Bloom level they perceive for each question.
