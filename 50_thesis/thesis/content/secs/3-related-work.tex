...
\object{Limited Content Adherence Evaluation} A persistent challenge in \ac{aqg} is the problem of hallucination, which is unacceptable in education \cite{yang_heuristic_2024}. \ac{llms} can generate misleading or irrelevant information which are not directly supported by e.g.\ the source text \cite{al_faraby_analysis_2024,duong-trung_bloomllm_2024}. While some studies mention this issue, the content adherence between a provided context and the generated questions still remains underexplored, as stated in the limitations of e.g.\ \cite{doughty_comparative_2024}. This thesis investigates this problem in RQ1, since this evaluation is crucial for generating reliable educational questions that can be answered accurately using the source material, as done in \cite{scaria_how_2024,maity_can_2025,blobstein_angel_2023}. Though highlighted in related research such as \cite{biancini_multiple-choice_2024}, it is important to inject knowledge directly into prompts to contrast hallucinations and give educators control over the source text.

\object{Bloom's Taxonomy Integration and Qualitative Evaluation} ... needed based on 3.2 primarily
% #TODO later

All these gaps highlight the need for a comprehensive evaluation framework that can evaluate modern \ac{llms} using current methods. The proposed framework below aims to bridge the research gaps and provide a foundation for future research in \ac{aqg} and \ac{llms}.